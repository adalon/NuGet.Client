...
namespace NuGet.Commands.FuncTest
{
    public class PackCommandTests
    {
        ...
        public void PackCommand_PackNewDefaultProject_NupkgExists()
        {
            using (var testDirectory = TestDirectory.Create())
            {
                ...
                // Act
                MsbuilldIntegrationTestUtility.CreateDotnetNewProject(testDirectory.Path, projectName, "-t lib");
                MsbuilldIntegrationTestUtility.RestoreProject(workingDirectory, projectName, string.Empty);
                MsbuilldIntegrationTestUtility.PackProject(workingDirectory, projectName, $"-o {workingDirectory}");
  ...
  <ItemGroup Condition=""'$(TargetFramework)' == 'net45'"">
    <PackageReference Include=""Newtonsoft.Json"" Version=""9.0.1"" />
                ...
                MsbuilldIntegrationTestUtility.CreateDotnetNewProject(testDirectory.Path, projectName);
  ...
  <ItemGroup>
    <ProjectReference Include=""..\ClassLibrary2\ClassLibrary2.csproj"" />
                ...
                MsbuilldIntegrationTestUtility.CreateDotnetNewProject(testDirectory.Path, projectName);
                MsbuilldIntegrationTestUtility.CreateDotnetNewProject(testDirectory.Path, referencedProject, "-t lib");
                File.WriteAllText(Path.Combine(workingDirectory, $"{projectName}.csproj"), projectFileContents);
                MsbuilldIntegrationTestUtility.RestoreProject(workingDirectory, projectName, string.Empty);
...
<package>
  ...
  <files>
    <file src=""bin/**/netstandard1.4/*.dll"" target=""CoreCLR/"" />
  </files>
</package>";
            using (var testDirectory = TestDirectory.Create())
            {
                ...
                MsbuilldIntegrationTestUtility.PackProject(workingDirectory, projectName, $"-o {workingDirectory} /p:NuspecFile=input.nuspec");
...
#if DEBUG
                    Assert.Equal("CoreCLR/bin/Debug/netstandard1.4/ClassLibrary1.dll", libItems[0]);
#else
                    Assert.Equal("CoreCLR/bin/Release/netstandard1.4/ClassLibrary1.dll", libItems[0]);
#endif
                }
